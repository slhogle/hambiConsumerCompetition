...
# Includes days 17, 21, 29, 45, 61. 
# This is the quasi equilibrium phase identified in the change point analysis

# These species are too rare to model accurately
rm_qeq <- c("HAMBI-0097", "HAMBI-0262", "HAMBI-1842", "HAMBI-1988", 
            "HAMBI-2159", "HAMBI-2160", "HAMBI-2164", "HAMBI-2443", 
            "HAMBI-2494", "HAMBI-2792", "HAMBI-3031", "HAMBI-3237")

counts_qeq <- counts %>%
  filter(day > 16) %>%
  filter(!(strainID %in% rm_qeq)) %>%
...
TrData <- TrData[!(row.names(TrData) %in% rm_qeq), ]

# phylogenetic tree
phyloTree <- read.tree(here::here("data", "phylotree.nwk"))
phyloTree <- drop.tip(phyloTree, rm_qeq)
...
ma_full <- Hmsc(Y           = Ya,
                XData       = XData, 
                XFormula    = ~cil*wrm + effort, 
                TrData      = TrData,
                TrFormula   = formula(paste0("~",paste(names(TrData),
                                                       collapse = "+"))),
                phyloTree   = phyloTree,
...
saveRDS(ma_full, here::here("data", "JSDM_unfit", "qeq_ma_full.rds"))
saveRDS(ma_envi, here::here("data", "JSDM_unfit", "qeq_ma_envi.rds"))
saveRDS(ma_time, here::here("data", "JSDM_unfit", "qeq_ma_time.rds"))
...
saveRDS(mp_full, here::here("data", "JSDM_unfit","qeq_mp_full.rds"))
saveRDS(mp_envi, here::here("data", "JSDM_unfit","qeq_mp_envi.rds"))
saveRDS(mp_time, here::here("data", "JSDM_unfit","qeq_mp_time.rds"))
