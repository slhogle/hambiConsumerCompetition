---
title: "Format quasi equilibrium, phase and prepare models for HMSC"
output:
  github_document:
    toc: yes
    toc_depth: 2
...
Includes days 17, 21, 29, 45, 61. This is the quasi equilibrium phase identified in the change point analysis
```{r}
...
1. A "FULL" model that includes both predation regime (environmental) and sequencing effort as fixed covariates as well as temporal random effects and hierarchical/repeated measure random effects.

2. A "ENVI" model that includes predation regime (environmental) and sequencing effort as fixed covariates but does not account for random effects.

3. A "TIME" model that excludes environmental covariates (sequencing effort only) and only models baseline abundance, but includes temporal and hierarchical/repeated measure random effects.
...
saveRDS(ma_full, here::here("models_unfit", "qeq_ma_full.rds"))
saveRDS(ma_envi, here::here("models_unfit", "qeq_ma_envi.rds"))
saveRDS(ma_time, here::here("models_unfit", "qeq_ma_time.rds"))
```
...
saveRDS(mp_full, here::here("models_unfit", "qeq_mp_full.rds"))
saveRDS(mp_envi, here::here("models_unfit", "qeq_mp_envi.rds"))
saveRDS(mp_time, here::here("models_unfit", "qeq_mp_time.rds"))
```
