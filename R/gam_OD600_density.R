...
# inverse Gaussian inverse w/ quadratic-link
with_seed(13513,
          m16 <- bam(f1, 
                     data = input,
...
# inverse Gaussian w/ inverse-link
with_seed(13513,
          m22 <- bam(f1, 
                     data = input, 
...
# inverse Gaussian w/ link-identity
with_seed(13513,
          m25 <- bam(f1, 
                     data = input, 
...
# Looks like Gaussian with log-link is best
...
# appears that k=9 is the best. Or at least it indicates there is no 
# longer any additional nonlinearity or structure in the residuals that 
# can be explained by a further smooth of time.
...
gamtabs(m05, caption='Summary of m1lnb', type = "latex") %>%
  write_lines(here::here("tables", "table_S3a.tex"))

# emmeans model contrasts
emmeans(m05, ~ treatment, data=input) %>%
  pairs(type = "response", adjust = "bonf") %>%
  xtable::xtable(type = "response") %>%
  print() %>%
  write_lines(here::here("tables", "table_S3b.tex"))

# Save model --------------------------------------------------------------
write_rds(m05, here::here("data", "GAM_OD600.rds"))
