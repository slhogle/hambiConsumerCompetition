...
# Load data ---------------------------------------------------------------
counts.corrected <-
  read_rds(here::here("data", "corrected_species_counts.rds")) %>%
  filter(sample != "HAMBI1")

expdesign <-
  read_tsv(here::here("data", "metadata.tsv"), col_types = "cccdc")

predator <-
  read_rds(here::here("data", "formatted_predator_prey_density.rds")) %>%
  filter(replicate != 1)
...
with_seed(5727458,
  ...
  left_join(expdesign) %>%
  mutate(sample1 = ifelse(str_detect(sample, "HAMBI"),
                          paste0(treatment, paste(replicate, paste0("d", day), sep = "_")),
                          sample)) %>%
  left_join(counts.corrected) %>%
  dplyr::select(sample, sample1, strainID, treatment, day, replicate, sflogcount,
                rlogcount = logcount, count.correct, count) %>%
  mutate(rlogcount = ifelse(is.na(rlogcount), sflogcount, rlogcount), 
...
with_seed(347812, rarefied <- counts.corrected %>%
  dplyr::select(-count) %>%
...
norm2 <- left_join(norm1, RA) %>%
  ...
  mutate(
    cil = factor(case_when(str_detect(treatment, "Pevo") ~ 2,
                           str_detect(treatment, "Panc") ~ 1,
                           TRUE ~ 0)),
    wrm = factor(case_when(str_detect(treatment, "HN") ~ 1,
                           TRUE ~ 0)),
    replicate = factor(replicate, levels=c("A", "B", "C", "D"))) %>%
  mutate(microcosmID = factor(paste(treatment, replicate, sep = "_")),
         treatment = factor(treatment, 
                            levels = c("H", "HN", "HPanc", "HPevo", "HNPanc", "HNPevo")),
         sample = factor(sample),
         strainID = factor(strainID)) %>%
  dplyr::select(sample, microcosmID, treatment, replicate, day, cil, wrm, strainID,
                count, count.correct, lograrecount, log10rab, sflogcount, rlogcount,
                PA) %>% 
  arrange(microcosmID, day)

write_rds(norm2, here::here("data", "normalized_corrected_species_counts.rds"))
